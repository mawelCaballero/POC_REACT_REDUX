...
export function searchAction( actionCriteria){
  ...
  return (dispatch) => {
      ...
      if (actionCriteria.actionType && actionCriteria.actionType === 'search') {
            var criteria = {
              param:{
                key: actionCriteria.criteria_id,
                value: actionCriteria.inputValue
              }
            }
            resourceFactory.getResponse(actionCriteria.resource, criteria).then(function (res) {
...
export function loadRenderingData(metamodelName, resourceName, url){
  return (dispatch) => {
    dispatch(loadMetamodel(metamodelName));
